...
function Quote() {
    ...
    const getAQuote = (event) => {
        ...
        api.get(`/customer/calculateamount/${order.parcel.weight}/${order.parcel.length}/${order.parcel.width}/${order.parcel.height}`)
            .then((response) => {
                amount = response.data;
                setMessage(`Hey! we will ship your parcel for ₹${response.data}. Remember this is an estimated quote and it may vary according to the precise addresses of the sender and the recepient`);
                openModal();
            })
            .catch((error) => {
            alert('Hey! there was an error getting you a quote. Please try again...');
});
    }
    ...
    return (
        <div className='container my-5'>
            ...
            <form>
                ...
                <button type='submit' className='btn btn-info' onClick={getAQuote}>Get a Quote</button>
            </form>
                ...
                <div className='modal-dialog'>
                    <div className='modal-content'>
                        ...
                        <div className='modal-footer'>
                            <button type='button' className='btn btn-outline-info' onClick={closeModal}>Get Another Quote</button>
                            <button type='button' className='btn btn-info' onClick={navigateToHomePage}>Go Back to Home Page</button>
                        </div>
                    </div>
                </div>
            </div>
